...
def run_trial(overrides, tag, config):
...
def main():
    ...
    ap.add_argument("--config", default=os.path.join("configs", "base.yaml"),
                    help="base config to mutate (env stays fixed; knobs are env-agnostic)")
    args = ap.parse_args()
    ...
    while args.trials == 0 or i < args.trials:
        ...
        if i % 5 == 4 and space.get("controls"):
            ...
            tag = f"{prefix}_{i:04d}_control"
        else:
            ov = sample(knobs, rng)
            tag = f"{prefix}_{i:04d}"
        try:
            metrics = run_trial(ov, tag, args.config)
        except subprocess.CalledProcessError as e:
            print(f"[trial {i}] FAILED: {e}")
            i += 1
            continue

        rec = {"trial": i, "tag": tag, "config": prefix, "overrides": ov, "metrics": metrics}
